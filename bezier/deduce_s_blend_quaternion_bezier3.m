...
% q0 = [[1,0,0]*sin(dis/2),cos(dis/2)]';
% q1 = [[0,cos(theta/5),sin(theta/5)]*sin(dis/12),cos(dis/12)]';
% q2 = [[cos(theta),sin(theta),0]*sin(dis/10),cos(dis/10)]';

axis1 = [0.9134    0.6324    0.0975]/norm([0.9134    0.6324    0.0975]);
axis2 = [0.6134    0.7324    0.2975]/norm([0.6134    0.7324    0.2975]);
axis3 = [0.9514    0.5324    0.2575]/norm([0.9514    0.5324    0.2575]);

q0 = [axis1*sin(dis/2),cos(dis/2)]';
q1 = [axis2*sin(dis/12),cos(dis/12)]';
q2 = [axis3*sin(dis/6),cos(dis/6)]';

ds = 0.001;
s = 0:ds:1;


[q,dq,ddq] = s_blend_quaternion_bezier3(q0,q1,q2,s);
[darc, ddarc] = s_darc_over_ds(dq,ddq);
[arc_est, darc_est,ddarc_est] = s_estimate_bezier3_arc(darc(1),ddarc(1),darc(end),ddarc(end),darc(0.5/ds + 1), s);
...
% plot(s(3:end)-3*ds/2,sqrt(sum(diff(diff(q'))'.*diff(diff(q'))'))/ds/ds)
% plot(s,ddarc)
