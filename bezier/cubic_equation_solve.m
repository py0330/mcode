function x = cubic_equation_solve(a,b,c,d)
%UNTITLED 此处提供此函数的摘要
%   此处提供详细说明
...
if(disc > 0)
    ...
    if(abs(q) < 1e-3 && 1)
        ...
        % dddg(0)
        real_q = q/(abs(r)^(2/3));
        s = 1.259921049894873 + 0.104993420824573 * real_q^3;
        s = s*abs(r)^(1/3)*sign(r);

%         s = r + sqrt(disc);
%         s = sign(s1)*abs(s1)^(1/3);
        ...
        % vpa(df(1e-10))
        t1 = r - sqrt(disc);
        t1 = sign(t1)*abs(t1)^(1/3);

        t = -0.79370052624990633724219768806982*real_q;
        t = t*abs(r)^(1/3)*sign(r);
    else
        s = r + sqrt(disc);
        s = sign(s)*abs(s)^(1/3);
        t = r - sqrt(disc);
...
%     x_below = xp2;
%     x_upper = x_rhs;
%     x(2) = newtor_raphson_binary_search(f, x_upper,x_below,10*eps);
%     x(3) = x(2);
%     return;
% end
